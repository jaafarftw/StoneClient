...
public class client extends RSApplet {
						...
						if (chatTypeView == 1 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210) {
								...
								if (byte0 == 1) {
									...
									modIcons[1].drawSprite(xPos, yPos - 13);
									xPos += 14;
								} else if (byte0 == 3) {
									modIcons[2].drawSprite(xPos + 1, yPos - 11);
									xPos += 14;
								} else if (byte0 == 4) {
						...
						if (chatTypeView == 2 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210) {
								...
								if (byte0 == 1) {
									...
									modIcons[1].drawSprite(k1 - 2, yPos - 13);
									k1 += 12;
								} else if (byte0 == 3) {
									modIcons[2].drawSprite(k1 - 1, yPos - 11);
									k1 += 12;
								} else if (byte0 == 4) {
	...
	void drawLoadingText(int i, String s) {
		...
		chatTextDrawingArea.drawText(0xffffff,
				"Stone-Kingdom is loading - please wait...",
				c1 / 2 - 26 - byte1, c / 2);
		int j = c1 / 2 - 18 - byte1;
	...
	public String getDocumentBaseHost() {
		...
		if (super.gameFrame != null) {
			return ""; // Stone-Kingdom.com <- removed for Jframe to work
		} else {
	...
	public void login(String s, String s1, boolean flag) {
		...
		try {
			...
			if (k == 6) {
				loginMessage1 = "Stone-Kingdom has been updated!";
	...
	public void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString(
					"Sorry, an error has occured whilst loading Stone-Kingdom",
					30, k);
			k += 50;
		...
		if (genericLoadingError) {
			...
			g.drawString("To play Stone-Kingdom make sure you play from", 50,
					100);
			g.drawString("http://www.Stone-Kingdom.com", 50, 150);
		}
		if (rsAlreadyLoaded) {
			aBoolean831 = false;
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString(
					"Error a copy of Stone-Kingdom already appears to be loaded",
					30, l);
			l += 50;
			g.setColor(Color.white);
		...
		for (int j2 = 0; j2 < i2; j2++) {
			...
			if (class9_1.type == 0) {
				...
				if (class9_1.scrollMax > class9_1.height) {
					...
					if (class9_1.id == 24322 || class9_1.id == 24423) {
						int members = 0;
	...
	public void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 0) {
			...
			chatTextDrawingArea.method382(0xffff00, c / 2,
					"Welcome to Stone-Kingdom", i, true);
			i += 30;
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
		...
		if (loginScreenState == 3) {
			...
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"Just login with any username", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"and password to begin your adventure!", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"for more information please visit:", k, true);
			k += 15;
			chatTextDrawingArea.method382(0xffffff, c / 2,
					"@gre@www.stone-kingdom.com", k, true);
			k += 15;
			int j1 = c / 2;
			int i2 = c1 / 2 + 50;
